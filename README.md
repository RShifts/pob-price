...
- `--crafted` 按文本匹配工艺词缀；`--crafted-empty` 不匹配文本、但按工艺词缀前后缀要求「空 1 前缀/空 1 后缀」（搜还有空位、可自己上工艺的装备）
- Web UI 有「工艺词缀」下拉：忽略 / 空1前缀或后缀（默认）/ 文本匹配
- 国际服固定只查「立即购买」（`sale_type=securable`）挂牌；国服查全部（国服后端不支持该过滤，会静默返回 0）
