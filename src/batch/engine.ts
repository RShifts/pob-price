...
export class BatchPriceEngine {
  ...
  private buildEntries(build: RawBuild, includeGems: boolean): ({ kind: "item"; slot: RawItemSlot; count: number; sig: string } | { kind: "gem"; gem: GemEntry; count: number; sig: string })[] {
  ...
  async run(build: RawBuild, opts: BatchOptions): Promise<BatchSummary> {
    ...
    const entries = this.buildEntries(build, opts.includeGems);
    ...
    for (const entry of entries) {
      ...
      try {
        results.push(await this.priceEntry(entry, statMap, conversion, opts));
      } catch (err) {
        results.push({
          kind: entry.kind,
          key: entry.sig,
